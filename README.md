...
数据页的“热量账本”支持手动食物热量记录、运动消耗估算以及当天摄入/消耗汇总。配置图片识别插件或服务端接口后可启用拍照估算和份量校正。算法、接口契约与医学边界见
[热量估算设计](docs/calorie-estimation.md)。

## 荣耀 500 桌面卡片

Android APK 包含 4×2 桌面健康卡片，可展示步数、心率、睡眠、热量结余和下一项计划。添加方式、数据同步机制与限制见
[荣耀 500 桌面健康卡片](docs/honor-500-widget.md)。

## 提醒说明

Android APK 使用 Capacitor 本地通知，在应用退出后仍由系统调度计划提醒。Android 13 及以上首次开启时需要允许通知；部分系统还需在应用设置中允许“闹钟和提醒”并关闭严格的后台省电限制。浏览器版本继续使用 Web Notification，无法保证浏览器彻底退出后仍准时唤醒。
