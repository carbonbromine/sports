(() => {
  ...
  function notificationBody(task) {
    const type = task.type === "meal" ? "饮食计划" : `${task.duration || 0} 分钟`;
    return `${task.time} · ${type}${task.note ? ` · ${task.note}` : ""}`;
  }
  ...
  async function sync(tasks, enabled) {
    ...
    const notifications = (Array.isArray(tasks) ? tasks : [])
      ...
      .map((task) => ({
        ...
        schedule: { at: reminderDate(task), allowWhileIdle: true },
        autoCancel: true,
        extra: { source: SOURCE, taskId: task.id }
      }));
